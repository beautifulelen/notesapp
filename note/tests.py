...
class TodoTest(UserAPITestCase):
    ...
    def create_todo(self):
        data = {'content':'remember the milk'}
    ...
    def test_create_todo(self):
        ...
        response = self.client.get('/notes/')
        expected_data = {'id':1, 'content':'remember the milk'}
    ...
    def test_update_todo(self):
        data = {'content':'remember the milk','done':True}

        self.register(self.user1)
        self.get_token(self.user1)
        self.create_todo()

        response = self.client.put('/notes/1', data)
        expected_code = status.HTTP_200_OK

        response = self.client.get('/notes/')
        expected_data = {'id':1, 'content':'remember the milk','is_favorite':True}
